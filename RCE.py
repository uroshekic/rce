...
class RCE:
    ...
    def alg(self, alg, t = 'raw'):
        if t == 'raw':
            for m in alg:
                if m in self.possible_rotations:
                    self.rotate(m)
                elif int(m) in self.possible_raw_moves:
                    self.turn(int(m))
        elif t == 'wca':
            self.last_used_wca_alg = alg
            raw = ''
            alg = alg.split(' ')
            for m in alg:
                if m in self.possible_rotations:
                    raw += m
                elif len(m) == 2 and m[0] in self.possible_rotations:
                    if m[1] == '2':
                        raw += m[0]*2
                    elif m[1] == "'":
                        raw += m[0]*3
                elif m in self.possible_turns:
                    raw += str(self.turns[m])
                elif len(m) == 2 and m[0] in self.possible_turns:
                    if m[1] == '2':
                        raw += str(self.turns[m[0]])*2
                    elif m[1] == "'":
                        raw += str(self.turns[m[0]])*3
            self.alg(raw)
